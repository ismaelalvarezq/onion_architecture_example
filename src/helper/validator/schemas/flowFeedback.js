...
const schemaFlowFeedback = Joi.object({
  id:
    Joi.string()
    .alter({
  ...
  idFlow:
    Joi.string()
    .alter({
      Post: (schema) => schema.required().messages({ "any.required": "La Id del flujo es requerida." }),
      Patch: (schema) => schema.forbidden(),
    })
    .messages({
      "string.base": "La Id de flujo debe ser un texto.",
      "string.empty": "La Id de flujo no puede estar vacío."
    }),
...
module.exports = { schemaFlowFeedback };